...
- Aspect : 공통 관심 사항을 모듈화한 것으로 로깅이나 보안 등의 기능을 Aspect로 정의할 수 있습니다.
- Join Point : Aspect가 적용될 수 있는 위치, 즉 메소드 호출이나 필드 접근과 같은 프로그램 실행의 특정 지점을 의미합니다.
- Advice : Aspect의 실제 구현체로 특정 Join Point에서 Aspect의 코드가 실행되는 시점을 정의합니다.
- Pointcut : Join point를 정의하는 표현식으로, 실제로 Advice가 적용될 위치를 선별하는 표현식입니다.
